...
class Detect_Traffic:
    def __init__(self, Model_path, img_size=640, conf_thres=0.5, iou_thres=0.45):
        ...
        self.crosswalk_points = []  # 횡단보도 좌표
        self.traffic_status = "신호등 탐지 안됨" #  신호등 상태
    ...
    def preprocess_traffic(self, frame):
        # preprocess
        ...
        if pred is not None:
            for p in pred:
                ...
                if class_name == "횡단보도":
                    ...
                    self.crosswalk_points.append(points)  # 리스트에 추가
                
                elif class_name in ["빨간불", "초록불"]: 
                    self.traffic_status = class_name    # 신호등 상태 저장
                   
        result_img = annotator.result()
         
        return result_img, points, self.traffic_status
